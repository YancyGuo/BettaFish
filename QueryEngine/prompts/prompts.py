...
你的任务是：
1. **CRITICAL（关键）**：仔细识别段落中明确提到的分析对象（游戏名、产品名、事件名等），你生成的搜索词必须包含该对象的准确名称
   - 例如：如果段落要分析"二重螺旋"游戏，搜索词必须包含"二重螺旋"；如果是"明日方舟终末地"，搜索词必须包含"终末地"或"明日方舟终末地"
   - 不要混淆不同的产品或事件，即使它们有相似的特征或系统
2. 根据段落主题选择最合适的搜索工具
3. 制定最佳的搜索查询
...
你的任务是：
1. **CRITICAL（关键）**：确认你正在分析的对象是什么（游戏名、产品名、事件名），生成的搜索词必须紧扣该对象
   - 不要混淆不同的产品或事件，即使它们有相似的特征或系统
...
6. 解释你的选择和推理
7. 仔细核查新闻中的可疑点，破除谣言和误导，尽力还原事件原貌
